...
experiment(){
	...
	STATS_DIR=$LOG_PATH_H/randomgraph-degree-lastweek/BLOCK${BLOCK_NO}/${EXP_NO}
...
cgroup_clean(){
	for node in `seq 167 170`; do echo $node; ssh dco-node$node 'echo 300M > /cgroup/mem_lmt_red/doit/memory.memsw.limit_in_bytes; echo 300M > /cgroup/mem_lmt_red/doit/memory.limit_in_bytes'; done
...
cgroup_clean
for CGROUP_LIMIT in 1365 2231 3963 4829 9158 13487 14353 16085 18000 35000; do  EXP_NO=1; RED_M=$((${CGROUP_LIMIT} - 300)) ; JVM_MAX_R=$((${CGROUP_LIMIT} - 500)) ;run ;done
cgroup_clean
for CGROUP_LIMIT in 1365 2231 3963 4829 9158 13487 14353 16085 18000 35000; do  EXP_NO=2; RED_M=$((${CGROUP_LIMIT} - 300)) ; JVM_MAX_R=$((${CGROUP_LIMIT} - 500)) ;run ;done

cgroup_clean
for CGROUP_LIMIT in 1365 2231 3963 4829 9158 13487 14353 16085 18000 35000; do  EXP_NO=3; RED_M=$((${CGROUP_LIMIT} - 300)) ; JVM_MAX_R=$((${CGROUP_LIMIT} - 500)) ;run ;done
#######################################
#
#INPUT=largeworld.txt
#
...
#
#cgroup_clean
#for CGROUP_LIMIT in  2000 6000 7000 8000 9000 10000 20000 30000; do  EXP_NO=1; RED_M=$((${CGROUP_LIMIT} - 300)) ; JVM_MAX_R=$((${CGROUP_LIMIT} - 500)) ;run ;done
#cgroup_clean
#for CGROUP_LIMIT in  2000 6000 7000 8000 9000 10000 20000 30000; do  EXP_NO=2; RED_M=$((${CGROUP_LIMIT} - 300)) ; JVM_MAX_R=$((${CGROUP_LIMIT} - 500)) ;run ;done
#cgroup_clean

#cp /HDD-2TB/wajih/Final-Experiments/com-detailed-randomgraph.sh $LOG_PATH_H/randomgraph-degree/
